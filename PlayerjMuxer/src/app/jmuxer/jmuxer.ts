...
export class JMuxmer extends Event {
  ...
  static isSupported(codec) {
    return ((window as any).MediaSource && (window as any).MediaSource.isTypeSupported(codec));
  }

  constructor(options) {
    super('jmuxer');
    (window as any).MediaSource = (window as any).MediaSource || (window as any).WebKitMediaSource;
    ...
    this.sourceBuffers = {};
    this.isMSESupported = !!(window as any).MediaSource;
